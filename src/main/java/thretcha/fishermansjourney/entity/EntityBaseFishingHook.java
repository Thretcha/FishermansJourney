...
public class EntityBaseFishingHook extends EntityFishHook
{
    private static final DataParameter<Integer> DATA_HOOKED_ENTITY = EntityDataManager.<Integer>createKey(EntityBaseFishingHook.class, DataSerializers.VARINT);
    private boolean inGround;
    private int ticksInGround;
    private EntityPlayer angler;
    private int ticksInAir;
    private int ticksCatchable;//the ticks during which a player has to react to reel in the fish
    private int ticksCaughtDelay;//ticks until a fish starts chasing the hook
    private int ticksCatchableDelay;//ticks till the fish has reached the hook and can be reeled in
    //ticksCaughtDelay + ticksCatchableDelay = the time it takes to catch a fish
    //minecraft tries to run at 20/s ticks/second
    //wow has a total fish time of up to 17 seconds and a reaction time of 3-4 seconds
    //17*20 = 340 ticks
    //ideally the reel in time is relatively constant and only the time till a fish chases the hook is stretched.
    private float fishApproachAngle;
    public Entity caughtEntity;
    private EntityBaseFishingHook.State currentState = EntityBaseFishingHook.State.FLYING;
    private int luck;
    private int lureSpeed;

    //my own defined variables for tweaking the fishing rod
    private int reactionTimeMin = 60;
    private int reactionTimeMax = 80;

    private int chaseTimeMin = 100;//20
    private int chaseTimeMax = 200;//80

    private int waitTimeMin = 200;
    private int waitTimeMax = 600;

    @SideOnly(Side.CLIENT)
    public EntityBaseFishingHook(World worldIn, EntityPlayer p_i47290_2_, double x, double y, double z)
    {
    ...
    public EntityBaseFishingHook(World worldIn, EntityPlayer fishingPlayer)
    {
    ...
    public void onUpdate()
    {
        ...
        else if (this.world.isRemote || !this.shouldStopFishing())
        {
            ...
            if (this.currentState == EntityBaseFishingHook.State.FLYING)
            {
                if (this.caughtEntity != null)
                {
                    ...
                    this.motionY = 0.0D;
                    this.motionZ = 0.0D;
                    this.currentState = EntityBaseFishingHook.State.HOOKED_IN_ENTITY;
                    return;
                }
                ...
                if (f > 0.0F)
                {
                    ...
                    this.motionY *= 0.2D;
                    this.motionZ *= 0.3D;
                    this.currentState = EntityBaseFishingHook.State.BOBBING;
                    return;
                }
            ...
            {
                if (this.currentState == EntityBaseFishingHook.State.HOOKED_IN_ENTITY)
                {
                    if (this.caughtEntity != null)
                    {
                        if (this.caughtEntity.isDead)
                        {
                            ...
                            this.currentState = EntityBaseFishingHook.State.FLYING;
                        }
                        else
                        {
                ...
                if (this.currentState == EntityBaseFishingHook.State.BOBBING)
                {
                    this.motionX *= 0.9D;
                    this.motionZ *= 0.9D;
    ...
    //called at the end of onUpdate
    //ticksCatchable=0; ticksCatchableDelay=0; ticksCaughtDelay =0; on first call of cathcingFish()
    private void catchingFish(BlockPos p_190621_1_)
    {
        ...
        /*
        //rain has a 25% chance of shortening the catch time by 1 tick
        if (this.rand.nextFloat() < 0.25F && this.world.isRainingAt(blockpos))
        {
            ++i;
        }
        //blocked sky has a 50% chance of increasing the catch time by 1 tick
        ...
        //a fish is chasing the hook
        else if (this.ticksCatchableDelay > 0)
        {
            this.ticksCatchableDelay -= i;

            //the fish hasn't reached the hook yet
            ...
            //the fish has reached the hook and is ready to be reeled in
            //the timer for the player to real in starts (ticksCatchable)
            else
            {
                ...
                this.ticksCatchable = MathHelper.getInt(this.rand, reactionTimeMin, reactionTimeMax);
            }
        }
        //smaller ticksCaughtDelay means more likely random splas particle effects spawn
        //ticksCaughtDelay reaches 0 => a fish will move towards the hook
        else if (this.ticksCaughtDelay > 0)
        {
            ...
            if (this.ticksCaughtDelay <= 0)
            {
                ...
                this.ticksCatchableDelay = MathHelper.getInt(this.rand, chaseTimeMin, chaseTimeMax);// look at this values changed 10 50
            }
        }
        else//all values are 0 on first call therefore this else statement is entered
        {
            this.ticksCaughtDelay = MathHelper.getInt(this.rand, waitTimeMin, waitTimeMax);// look at this values changed 20 120
            //this.ticksCaughtDelay -= this.lureSpeed * 20 * 5;//statement not necersarry because we set lurespeed to 0
        }
    }
